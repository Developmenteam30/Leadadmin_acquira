  ...
  <div>
    ...
    <div class="container-fluid">
      ...
      <p>
        ...
        <router-link to="/outgoing-feeds" class="btn btn-primary" style="margin-left: 10px;">
          Outgoing Feeds
        </router-link>
      ...
      <div v-else-if="companyGroups.length === 0">
        <p>No outgoing ping feeds found.</p>
      </div>

      <div v-else>
        <h4>Outgoing Phone Pre-Ping Feeds</h4>
      ...
      <!-- Outgoing Ping Feed Modals -->
      <Teleport to="body">
        <!-- Populations Modal -->
        <div v-show="populationsModal.show" class="outgoing-feed-modal" tabindex="-1" @click.self="closePopulationsModal">
          <div class="modal-dialog modal-lg" style="max-width: 95%;" @click.stop>
            <div class="modal-content">
              ...
              <div class="modal-body">
                ...
                <template v-else>
                  <p>
                    <button type="button" class="btn btn-primary btn-sm" @click="openAddPopulationModal">Add a new population parameter</button>
                  </p>
                    ...
                    <table class="table table-bordered table-condensed table-striped">
                      ...
                      <tbody>
                        <tr v-for="p in populations" :key="p.idAssoc">
                          ...
                          <td>
                            <label class="switch">
                              <input type="checkbox" :checked="p.enabled === '1'" @change="togglePopulation(p.idAssoc)" />
                              <span class="slider"></span>
                            </label>
        ...
        <div v-show="addPopulationModal.show" class="outgoing-feed-modal add-population-modal" tabindex="-1" @click.self="closeAddPopulationModal">
          <div class="modal-dialog modal-lg" @click.stop>
            <div class="modal-content">
              <div class="modal-header">
                <h4 class="modal-title">{{ addPopulationModal.editId ? 'Edit population' : 'Add a new population parameter' }}</h4>
                <button type="button" class="close" @click="closeAddPopulationModal">&times;</button>
              </div>
              <div class="modal-body">
                ...
                <div class="add-pop-row">
                  ...
                  <div class="add-pop-content">
                    <div class="add-pop-queue-options">
                      <label class="add-pop-queue-opt"><input type="radio" v-model="addPopulationModal.queueType" value="livedata" /> Live Data</label>
                      <label class="add-pop-queue-opt"><input type="radio" v-model="addPopulationModal.queueType" value="queue" /> Standard Queue</label>
                      <label class="add-pop-queue-opt"><input type="radio" v-model="addPopulationModal.queueType" value="waterfall" /> Waterfall Live Standard</label>
                      <label class="add-pop-queue-opt"><input type="radio" v-model="addPopulationModal.queueType" value="waterfallLimit" /> Waterfall Limit &amp; Queue</label>
                      <label class="add-pop-queue-opt"><input type="radio" v-model="addPopulationModal.queueType" value="waterfallLimitLive" /> Waterfall Limit Live</label>
                    </div>
                  </div>
                </div>
                <div class="add-pop-row">
                  ...
                  <div class="add-pop-content">
                    <input v-model.number="addPopulationModal.waterfallPriority" type="number" min="0" max="65535" class="form-control add-pop-input add-pop-input-sm" />
                  </div>
                </div>
                <div class="add-pop-row">
        ...
        <div v-show="testRecordModal.show" class="outgoing-feed-modal" tabindex="-1" @click.self="closeTestRecordModal">
          <div class="modal-dialog modal-lg" @click.stop>
            <div class="modal-content">
              <div class="modal-header">
...
export default {
  name: 'OutgoingFeedsPingManagement',
  components: {
    Navigation,
    OutboundFeedForm,
    QuickJump,
  ...
  setup() {
    ...
    const populationsModal = reactive({ show: false, idFeedOut: null, feedLabel: '', feedDescription: '' });
    ...
    const fetchFeeds = async () => {
      ...
      try {
        ...
        const response = await axios.get('/api/outbound-feeds/ping', { params });
        if (response.data.status === 1) {
          companyGroups.value = response.data.data || [];
        }
      } catch (error) {
        console.error('Error fetching outgoing ping feeds:', error);
        companyGroups.value = [];
      } finally {
        loading.value = false;
      }
      ...
      try {
        ...
        if (response.data.status === 1) {
          ...
          Object.assign(editingFeed, {
            ...
            timezone: feedData.timezone || 'UTC',
    ...
    const handleAddFeed = async () => {
      ...
      try {
        const payload = {
          ...
          costKey: newFeed.costKey?.trim() || null,
    ...
    const handleUpdateFeed = async () => {
      ...
      try {
        const payload = {
          ...
          costKey: editingFeed.costKey?.trim() || null,
...
.add-population-modal .modal-dialog { width: 800px; max-width: 95%; }
.add-population-modal .modal-content { border: 1px solid #ddd; border-radius: 4px; }
.add-population-modal .modal-header { background: #072f5f; color: #fff; border-bottom: none; padding: 12px 15px; border-radius: 4px 4px 0 0; }
.add-population-modal .modal-header .modal-title { font-size: 16px; font-weight: 600; }
.add-population-modal .modal-header .close { color: #fff; opacity: 0.9; text-shadow: none; }
.add-population-modal .modal-header .close:hover { opacity: 1; }
.add-population-modal .modal-body { padding: 20px; max-height: 70vh; overflow-y: auto; }
.add-population-modal .modal-footer { padding: 12px 20px; border-top: 1px solid #eee; background: #f9f9f9; }
.add-pop-row { display: flex; align-items: flex-start; margin-bottom: 0; padding-bottom: 0; }
.add-pop-row + .add-pop-row { margin-top: 18px; padding-top: 18px; border-top: 1px solid #eee; }
.add-pop-heading { flex: 0 0 180px; font-weight: 600; color: #333; font-size: 14px; padding-right: 20px; padding-top: 2px; }
.add-pop-content { flex: 1; min-width: 0; }
.add-pop-desc { color: #666; font-size: 13px; margin: 0 0 10px 0; }
.add-pop-radios { margin-bottom: 10px; }
.add-pop-radio { display: inline-block; margin-right: 20px; font-weight: normal; cursor: pointer; font-size: 14px; }
.add-pop-radio input { margin-right: 6px; vertical-align: middle; }
.add-pop-select, .add-pop-input { max-width: 450px; margin-top: 6px; }
.add-pop-input-sm { max-width: 180px; }
.add-pop-queue-options { margin-top: 6px; }
.add-pop-queue-opt { display: block; margin-bottom: 8px; font-weight: normal; cursor: pointer; font-size: 14px; }
.add-pop-queue-opt input { margin-right: 8px; vertical-align: middle; }
.add-pop-checkbox { cursor: pointer; font-weight: normal; }
.add-pop-checkbox input { margin-right: 6px; }
.add-pop-readonly { margin: 8px 0 0 0; padding: 6px 10px; background: #f5f5f5; border-radius: 4px; font-size: 14px; }
