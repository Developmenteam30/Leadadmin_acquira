...
class Leads
{
    ...
    public function getUnpaidLedger(
        $type,
        $userId = null,
        $distinctColumn = null,
        $distinctValue = null
        ...
        if (!empty($userId)) {
            ...
            $sql .= "AND (l.paymentDate IS NULL ";
            $sql .= "OR l.paymentAmount IS NULL ";
            $sql .= "OR l.paymentMethod IS NULL ) ";
        }
        if (!empty($distinctColumn) && !empty($distinctValue)) {
            $sql .= "AND " . $distinctColumn . " = ? ";
            $params[] = $distinctValue;
...
//			$sql .= "ORDER BY fp.waterfallPriority DESC";
    ...
    {
        if (empty($lookbackPeriod) || is_numeric($lookbackPeriod) === false || $lookbackPeriod < 0) {
            $lookbackPeriod = 120;
        }
...
//                $query = $this->db->prepare( "ALTER TABLE archive.{$table} ADD COLUMN accepted TINYINT UNSIGNED DEFAULT 1" );
    ...
    public function logError($message, $db = false, $email = true)
    {
        ...
        if ($email) {
            ...
            $time = @file_get_contents(SITE_ROOT . DS . "error" . DIRECTORY_SEPARATOR . "email-stamp"); // Add . DS . here
            if ($time === false || ($time < (time() - 60))) {
                file_put_contents(SITE_ROOT . DS. "error" . DIRECTORY_SEPARATOR . "email-stamp", time()); // Add . DS . here
            } else {
                return;
            }
