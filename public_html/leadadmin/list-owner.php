...
if (isset($_REQUEST['a'])) {
    ...
    switch ($_REQUEST['a']) {
        ...
        case 'invoice_status':
            ...
            $leads->setInvoiceDetails($_REQUEST['date'], $_REQUEST['idCompany'], !empty($_REQUEST['invoiceNumber']) ? $_REQUEST['invoiceNumber'] : '', !empty($_REQUEST['paymentDate']) ? $_REQUEST['paymentDate'] : '',
                !empty($_REQUEST['userId']) ? $_REQUEST['userId'] : '');
            $result['status'] = 1;
        ...
        case 'dialog_revenue_listowners':
            ...
            <p><strong>Report Date:</strong>
                ...
                if (!empty($idCompany)) {
                    ...
                    printf('<input class="btn btn-primary btn-sm" type="button" value="Copy values from last month" onclick="copyRevenue( \'%s\', \'%s\', \'%s\' )" /> ', $reportDateObj->format('Ym'), $reportDate,
                        $idCompany);
            ...
            if ($mappings) {
                ...
                foreach ($mappings as $mapping) {
                    ...
                    printf("\t\t<td class=\"revenue\" data-tf-sortKey=\"%s\"><input type=\"number\" min=\"0\" max=\"9999\" step=\"0.01\" id=\"%s\" name=\"%s\" value=\"%s\" /></td>\n",
                        (empty($mapping['revenue']) ? '0' : htmlspecialchars($mapping['revenue'])), 'A' . ++$row,
                        htmlspecialchars(base64_encode($reportDate . '|' . $mapping['idFeedIn'] . '|' . $mapping['idFeedOut'] . '|' . $mapping['url'])),
