...
<h2>Open Receivables Report</h2>

<?php
	$monthIn = !empty( $_REQUEST['month'] ) ? $_REQUEST['month'] : null;
	$monthSelected = null;
	$months = $leads->getUnpaidLedger( 1, null, 'LEFT(ledgerMonth,7)', null );
	...
	if( empty( $monthSelected ) ) {
		...
		if( strlen( $monthSelected ) == 4 ) {
			$entries = $leads->getUnpaidLedger( 1, null, 'LEFT(ledgerMonth,4)', $monthSelected );
		} else if( preg_match( '/^(20[0-9]{2})-Q([1-4])$/', $monthSelected, $matches ) ) {
			$entries = $leads->getUnpaidLedger( 1, null, 'CONCAT(LEFT(ledgerMonth,4),QUARTER(ledgerMonth))', $matches[1] . $matches[2] );
		} else {
			$entries = $leads->getUnpaidLedger( 1, null, 'LEFT(ledgerMonth,7)', $monthSelected );
		}
...
<button type="button" class="btn btn-primary btn-xs" data-toggle="modal" data-backdrop="static" data-target="#editofflineledger" data-ledger-id="<?php echo $entry->ledgerId; ?>">Edit</button>
<?php } else if( 'ledger_phones' === $entry->source ) { ?>
<button type="button" class="btn btn-primary btn-xs" data-toggle="modal" data-backdrop="static" data-target="#editphoneledger" data-ledger-id="<?php echo $entry->ledgerId; ?>">Edit</button>
<?php } else if( 'email' === $entry->source ) { print "&nbsp;"; ?>
<?php } else { ?>
<button type="button" class="btn btn-primary btn-xs" data-toggle="modal" data-backdrop="static" data-target="#editledger" data-ledger-id="<?php echo $entry->ledgerId; ?>">Edit</button></td>
<?php } ?>
		</tr>
<?php
					}
