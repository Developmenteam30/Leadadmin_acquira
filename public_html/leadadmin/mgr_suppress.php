...
if (isset($_REQUEST['a'])) {
    ...
    switch ($_REQUEST['a']) {
        ...
        case 'Add':
            ...
            $newFile = UPLOADS_DIR . hash('sha256', $_FILES['suppress_file']['tmp_name']);
            if (move_uploaded_file($_FILES['suppress_file']['tmp_name'], $newFile) !== true) {
                $result['error'] = 'Cannot move uploaded file for processing';
            }

            $jobId = $leads->addJob('email-suppression', $_REQUEST['idFeedIn'], serialize($_REQUEST), $newFile, $cnt);
            if (null === $jobId) {
                ...
                switch ($_REQUEST['type']) {
                    case 'single':
                        if ($c && ( //email must not be empty.
                            ...
                            $c = false;
                            $result['error'] = 'Email field cannot be empty.';
                        }
                        if ($c) { //Passed initial validation
                            if (strpos($_REQUEST['email'], '@') !== false && !filter_var($_REQUEST['email'],
                    ...
                    case 'multiple':
                        if ($c && ( //email array must not be empty.
                            ...
                            $c = false;
                            $result['error'] = 'Emails field cannot be empty.';
                        }
                        if ($c) { //Passed initial validation
...
$title = 'Email Suppressions Manager';
    ...
    function processSuppression(type) {
        switch (type) {
            ...
            case 'multiple':
                ...
                if (emaillist == '') {
                    alert("Email list must not be empty.");
                    return false;
                } else {
...
<div class="container-fluid">
    ...
    <div id='controls' class='fl50'>
        ...
        <div id='resultImport'><?php if (!empty($_REQUEST['a']) && 'Add' == $_REQUEST['a']) {
                print "<p style=\"color: blue;\">File import status: {$result['error']}</p><p>Successes: {$counts['success']}</p><p>Invalid emails: {$counts['invalid']}</p><p>Duplicates: {$counts['dupe']}</p><p>Failures: {$counts['failures']}</p>";
            } ?></div>
