...
<div class="mainContainer">
    ...
    if (empty($id) || empty($type)) {
        ...
        if ($records === false) {
            ...
            <h3>Last 100 rejections for <?php echo Display::escHtml($type); ?>
                feed: <?php echo Display::escHtml($label); ?></h3>

            <?php if ($offset > 0) { ?>
                <p style="display: inline-block;margin-right:10px;"><?php printf('<a class="btn btn-primary btn-sm"href="mgr_rejections.php?type=%s&amp;id=%s&amp;label=%s&amp;offset=%d">Previous page</a>',
                        urlencode($type), urlencode($id), urlencode($label), intval($offset - 100)); ?></p>
            <?php } ?>
            <p style="display: inline-block;"><?php printf('<a class="btn btn-primary btn-sm" href="mgr_rejections.php?type=%s&amp;id=%s&amp;label=%s&amp;offset=%d">Next page</a>',
                    urlencode($type), urlencode($id), urlencode($label), intval($offset + 100)); ?></p>
            <br>
                ...
                <?php foreach ($records as $record) { ?>
                    <tr>
                        <?php if ('outbound' == $type) { ?>
                            <td><a href="#" data-toggle="modal" data-backdrop="static" data-target="#modal-postparams"
                                   data-feed-id="<?php echo intval($id); ?>"
                                   data-record-id="<?php echo intval($record['idRecord']); ?>"
                                   data-accepted="<?php echo intval($record['accepted']); ?>"
                                   data-result="<?php echo Display::escHtml($record['result']); ?>"><?php echo Display::escHtml($record['timestampConverted']); ?></a>
                            </td>
                        <?php } else { ?>
