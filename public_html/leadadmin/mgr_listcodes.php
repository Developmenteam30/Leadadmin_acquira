...
$forceMysqlLogFile = SITE_ROOT . "error" . DIRECTORY_SEPARATOR . "log_listcodes";
	...
	switch( $_REQUEST['d'] ) {
							...
							foreach( $listcodes as $listcode ) {
								...
								<tr>
									...
									<td>
										...
										?>
										<p><textarea id="add_urls_<?php echo $listcode->idListcode; ?>" style="width:95%; height:75px;"></textarea>
											<button onclick="saveUrls(<?php echo $listcode->idListcode; ?>, <?php echo $company->idCompany; ?>)">Add new URLs</button>
										</p>
									</td>
									<td>
		...
		case 'dialog_editlistcode':
			...
			if( $listcode === false ) {
				...
				<?php
				exit;
			} else if( !is_object( $listcode ) && $listcode == 0 ) {
				?>
		...
		case 'dialog_newlistcode':
			...
			foreach( $props as $prop ) {
				...
				} else if( isset( $_REQUEST['options'][$prop] ) ) {
